    ...
    void searchAndParam() {
        Member findMember = queryFactory
                ...
                .where(
                        ...
                        member.age.eq(10)   //member.username.eq("member1").and(member.age.eq(10))
                )
                .fetchOne();
        assertThat(findMember.getUsername()).isEqualTo("member1");
    }
...
//        List<Member> content = results.getResults();
    ...
    public void join_on_no_relation(){
        ...
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                //.leftJoin(member.team, team) -> id 랑 매칭 없으면 그냥 조건
    ...
    public void subQuery() {
        ...
        QMember memberSub = new QMember("memberSub");// 서브쿼리에서 as 중첩 방지
    ...
    @Test
    public void findDtoByField() { //geter, setter 없어도 됨
        List<MemberDto> result = queryFactory
    ...
    @Test
    public void findUserDto() { //geter, setter 없어도 됨
    ...
    /**
     * 장점: QueryProjection는 컴파일 오류 Constructor는 런타임 오류
     * 단점: Q파일 생성 해야함, *Dto가 querydsl에 의존성이 생김*
     */
    @Test
    ...
    private List<Member> searchMember1(String usernameCond, Integer ageCond) {
        ...
        //BooleanBuilder builder= new BooleanBuilder(member.username.eq(usernameCond)); // 초기값 넣을 수 있음(필수 값일 때)
